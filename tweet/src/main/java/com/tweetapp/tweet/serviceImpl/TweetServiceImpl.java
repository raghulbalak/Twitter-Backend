...
public class TweetServiceImpl implements TweetService {
	...
	public RegisterResponseDTO updateTweet(String id, String tweet) throws Exception {
		...
		try {
			Optional<Tweet> update = tweetsRepository.findById(id);
			if (update.isPresent()) {
				update.get().setTweet(tweet);
				tweetsRepository.save(update.get());
	...
	public RegisterResponseDTO replyTweet(String userId, String id, String reply) throws Exception {
		...
		try {
			...
			if (user != null) {
				...
				LocalDateTime now = LocalDateTime.now();
				if (tweet.isPresent()) {
					List<Reply> replies = tweet.get().getReplies();
					...
					tweet.get().setReplies(replyList);
	...
	public RegisterResponseDTO likeTweet(String userId, String id, Boolean status) throws Exception {
		...
		try {
			...
			if (user != null) {
				Optional<Tweet> tweet = tweetsRepository.findById(id);
				if (tweet.isPresent()) {
					List<Like> likes = tweet.get().getLikes();
